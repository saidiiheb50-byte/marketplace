# Step 5 & 6 Quick Guide - Railway Database & Backend URL

## 📋 STEP 5: Setup Database (Choose ONE method)
...
2. **Find MySQL Service**
   - You should see 2 services: Your backend + MySQL
   - Click on the **MySQL service** (the one with database icon)

3. **Open Data Tab**
   - Click **"Data"** tab at the top
   - You'll see a query editor/console

4. **Copy SQL Schema**
   - Open file: `server/database/schema-railway.sql` from your project
   - **Select ALL** (Ctrl+A) and **Copy** (Ctrl+C)
   - The file is also shown below ⬇️

5. **Paste and Run**
   - Paste into Railway's MySQL console
   - Click **"Run"** or press Enter
   - Wait for "Query OK" messages

6. **Verify It Worked**
   - Run this query: `SHOW TABLES;`
...
1. **Go to Backend Service**
   - In Railway dashboard, click on your **backend service** (not MySQL)

2. **Check Deployment Status**
   - Go to **"Deployments"** tab
   - Make sure latest shows **"Active"** ✅
...
5. **Test It**
   - Open in browser: `https://your-app-name.up.railway.app/api/health`
   - Should show JSON with database status

6. **Update Vercel**
   - Go to Vercel → Settings → Environment Variables
   - Set `VITE_API_URL` = `https://your-app-name.up.railway.app/api`
   - Redeploy Vercel
...
- ✅ That's OK! Tables are already created. Skip to Step 6.
...
## 📝 SQL Schema to Copy (from schema-railway.sql)

Copy everything below this line and paste into Railway MySQL console:

```sql
-- Béja Marketplace Database Schema for Railway
...
DESCRIBE users;
```
Should show all columns including user_type and account_status.
